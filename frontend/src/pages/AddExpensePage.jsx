...
const categories = ["Food", "Travel", "Shopping", "Bills", "Subscriptions", "Other"];
const maxUploadSizeMb = Number(import.meta.env.VITE_MAX_UPLOAD_SIZE_MB || 5);

function AddExpensePage() {
  const [form, setForm] = useState({ amount: "", category: "", description: "", date: dayjs().format("YYYY-MM-DD") });
  ...
  const submitQuick = async (e) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Could not parse and save the expense."));
    }
  };
  ...
  const processFile = (fileToProcess) => {
    ...
    if (fileToProcess.size > maxUploadSizeMb * 1024 * 1024) {
      setError(`File is too large (max ${maxUploadSizeMb}MB). Please split your statement.`);
      setFile(null);
    }
  };
  ...
  const submitCsv = async (e) => {
    ...
    try {
      ...
      if (skippedCount > 0) summaryParts.push(`Skipped ${skippedCount} un-matched rows.`);
      if (duplicateCount > 0) summaryParts.push(`Ignored ${duplicateCount} duplicates.`);
      ...
      setError(getApiErrorMessage(err, "Statement upload failed. Please verify the file format."));
      setUploadProgress(0);
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fade">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <form onSubmit={submitManual} className="mc-card p-6 space-y-4 mc-stagger-1">
          <div>
            <h3 className="text-lg font-bold">Manual Entry</h3>
            <p className="text-sm text-[var(--text-muted)] mt-1">Add a single transaction</p>
          </div>
          
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-[var(--text-secondary)]">Amount (₹)</label>
            <input type="number" step="0.01" required placeholder="0.00" className="mc-input" value={form.amount} onChange={(e) => setForm((s) => ({ ...s, amount: e.target.value }))} />
          </div>
          
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-[var(--text-secondary)]">Category</label>
            <select className="mc-input bg-white" value={form.category} onChange={(e) => setForm((s) => ({ ...s, category: e.target.value }))}>
          ...
          <div className="space-y-1.5">
            ...
            <input required placeholder="Merchant or item name" className="mc-input" value={form.description} onChange={(e) => setForm((s) => ({ ...s, description: e.target.value }))} />
          </div>
          
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-[var(--text-secondary)]">Date</label>
            <input type="date" required className="mc-input" value={form.date} onChange={(e) => setForm((s) => ({ ...s, date: e.target.value }))} />
          </div>
        ...
        <div className="space-y-5">
          <form onSubmit={submitQuick} className="mc-card p-6 space-y-4 mc-stagger-2">
            <div>
              <h3 className="text-lg font-bold">Natural Language Parsing</h3>
              <p className="text-sm text-[var(--text-muted)] mt-1">Let AI extract the details for you</p>
            </div>
          ...
          <form onSubmit={submitCsv} className="mc-card p-6 space-y-4 mc-stagger-3">
            <div>
              <h3 className="text-lg font-bold">Statement Upload</h3>
              <p className="text-sm text-[var(--text-muted)] mt-1">Bulk import via CSV or Excel</p>
            </div>
                ...
                {file ? (
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-[var(--text-muted)] mt-1">{(file.size/1024).toFixed(1)} KB • Click to replace</p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-[var(--text-muted)] mt-1 max-w-[200px] mx-auto">
                        Supports standard bank formats, UPI, Paytm, and custom CSVs.
                      </p>
                    </div>
                  </>
                )}
            ...
            {isUploading && (
              <div className="space-y-2 mt-4 animate-fade">
                ...
                <div className="mc-progress-track">
                  <div className="mc-progress-fill" style={{ width: `${uploadProgress}%` }} />
                </div>
              </div>
            )}
