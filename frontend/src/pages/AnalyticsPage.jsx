...
function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    async function loadAnalytics() {
      try {
        ...
        const [{ data: dashboardData }, { data: insightData }] = await Promise.all([
  ...
  const topCategoryShare = topCategory && dashboard?.totalSpend
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh] animate-fade">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-[var(--text-muted)]">Crunching the numbers...</p>
        </div>
      </div>
    );
  }
        ...
        <>
          <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 animate-slide mc-stagger-1">
            <StatCard 
              ...
              icon={Activity} 
              tone={insight.financialScore > 70 ? "success" : insight.financialScore < 40 ? "danger" : "warning"}
            />
            <StatCard 
              title="Risk Level" 
              value={insight.diagnostics?.riskLevel || "Low"} 
              icon={ShieldAlert}
              tone={insight.diagnostics?.riskLevel === "High" ? "danger" : insight.diagnostics?.riskLevel === "Medium" ? "warning" : "success"}
            />
            <StatCard 
              title="Projected End" 
          ...
          <section className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-slide mc-stagger-3">
            <div className="mc-card p-6">
              ...
              <p className="mc-section-subtitle mb-5">Where your money went this month.</p>
              
              <div className="space-y-1">
                {dashboard.topCategories.length ? (
                  dashboard.topCategories.map((item, index) => {
                    const percentage = dashboard.totalSpend ? (item.total / dashboard.totalSpend) * 100 : 0;
                    return (
                      <div key={item.category} className="group flex flex-col pt-3 pb-3 border-b border-[var(--border-light)] last:border-0">
                  ...
                  <p className="text-sm text-[var(--text-muted)] p-5 border border-dashed rounded-xl text-center">No category data yet.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="mc-card p-6 flex-1">
                ...
                <p className="mc-section-subtitle mb-4">Recurring costs bound to repeat.</p>
                
                <ul className="space-y-2">
                  {dashboard.subscriptions.length ? (
                    dashboard.subscriptions.map((item) => (
                      <li key={`${item.description}-${item.renewalDate}`} className="flex justify-between items-center bg-[var(--bg-base)] rounded-xl border border-[var(--border-light)] p-3 hover:border-[var(--border)] transition-colors">
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                            <Repeat size={14} className="text-indigo-600" />
                          </div>
                          <div className="min-w-0">
                            <span className="font-semibold capitalize text-sm text-[var(--text-primary)] truncate block">{item.description}</span>
                            <span className="text-xs text-[var(--text-muted)]">Renews {item.renewalDate}</span>
                          </div>
                        </div>
                        <span className="font-bold text-sm shrink-0">₹{Number(item.monthlyCost).toFixed(2)}</span>
                      </li>
                    ))
                  ) : (
                    <li className="text-sm text-[var(--text-muted)] p-5 border border-dashed rounded-xl text-center">No fixed subscriptions found.</li>
                  )}
                </ul>
              </div>
              ...
              <div className="mc-card p-6 flex-1 border-amber-200">
                ...
                <p className="mc-section-subtitle mb-4">Unusual spikes deserving review.</p>
                
                <ul className="space-y-2">
                  {dashboard.anomalies.length ? (
                    dashboard.anomalies.map((item) => (
                      <li key={item._id} className="flex justify-between items-center bg-amber-50/50 rounded-xl border border-amber-100 p-3 hover:border-amber-200 transition-colors">
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center shrink-0">
                            <Zap size={14} className="text-amber-600" />
                          </div>
                          <span className="font-semibold text-sm text-amber-900 truncate">{item.description}</span>
                        </div>
                        <span className="font-bold text-sm text-amber-700 shrink-0">₹{Number(item.amount).toFixed(2)}</span>
                      </li>
                    ))
                  ) : (
...
// Need explicitly imported Sparkles
