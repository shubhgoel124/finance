...
function ChatPage() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Hi! Ask me anything about your spending. For example:\n• \"Where did I spend the most this month?\"\n• \"Suggest a weekly savings target\"\n• \"Compare my spending with last month\"",
      followUps: ["Top expenses this month", "My largest category", "Savings advice"],
      context: []
    }
  ]);
  ...
  const send = async (nextQuery) => {
    ...
    try {
      ...
      setQuery("");
      // Add slight delay before focusing to let animations run
      setTimeout(() => inputRef.current?.focus(), 100);
    } catch (err) {
      const message = getApiErrorMessage(err, "I could not generate an answer right now. Check the API configuration and try again.");
      setError(message);
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content: "I'm having trouble connecting to my brain right now. Please check if the API key is configured correctly and try again.",
          followUps: [],
          context: []
        }
      ]);
      ...
      <div className="flex-1 min-h-0 mc-card flex flex-col overflow-hidden mb-6">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
          {messages.map((message, index) => {
            ...
            return (
              <div key={`${message.role}-${index}`} className={`flex ${isUser ? "justify-end" : "justify-start"} animate-slide`}>
                <div className={`flex gap-3 max-w-[85%] md:max-w-[75%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                  ...
                  <div className="space-y-3">
                    <div className={`px-4 py-3 whitespace-pre-wrap text-sm leading-relaxed
                      ...
                      <details className="mt-1 group">
                        ...
                        <div className="mt-2 text-xs bg-[var(--bg-muted)] rounded-lg p-3 space-y-2 border border-[var(--border-light)] max-h-40 overflow-auto">
                          {message.context.map((item, contextIndex) => (
                            <div key={`${item.description}-${contextIndex}`} className="flex justify-between gap-4 py-1">
                              <span className="text-[var(--text-secondary)] font-medium truncate">{item.description}</span>
                              <span className="font-semibold shrink-0">₹{Number(item.amount || 0).toFixed(2)}</span>
                            </div>
                          ))}
                        </div>
                      </details>
